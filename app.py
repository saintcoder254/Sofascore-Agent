...
adapter=FeedFusionAdapter(BASE,TIMEOUT); store=Store(DB); fresh=FreshnessGate(STALE_AFTER); conflict=ConflictGate(); evolution=EvolutionAgent(store,EVOLUTION_MIN_SAMPLES); research=ResearchEngine(store); learning=ClosedLoopLearning(store,evolution); source_learning=SourceLearningAgent(store,SOURCE_MIN_SAMPLES)
...
async def source_learning_loop():
    state["source_learning_running"]=True; logger.info("SOURCE_LEARNING_LOOP_STARTED interval_seconds=%s min_samples=%s",SOURCE_LEARNING_SECONDS,SOURCE_MIN_SAMPLES)
    while True:
