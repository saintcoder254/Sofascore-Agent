...
POLL_SECONDS=int(os.getenv("POLL_SECONDS","60")); STALE_AFTER=int(os.getenv("STALE_AFTER_SECONDS","180")); BASE=os.getenv("SOFASCORE_BASE","https://www.sofascore.com/api/v1"); DB=os.getenv("DATABASE_PATH","matchmaster.db"); TIMEOUT=float(os.getenv("HTTP_TIMEOUT_SECONDS","15")); EVOLUTION_SECONDS=int(os.getenv("EVOLUTION_SECONDS","21600")); EVOLUTION_MIN_SAMPLES=int(os.getenv("EVOLUTION_MIN_SAMPLES","100")); RESEARCH_SECONDS=int(os.getenv("RESEARCH_SECONDS","21600")); LEARNING_SECONDS=int(os.getenv("LEARNING_SECONDS","300"))
adapter=FeedFusionAdapter(BASE,TIMEOUT); store=Store(DB); fresh=FreshnessGate(STALE_AFTER); conflict=ConflictGate(); evolution=EvolutionAgent(store,EVOLUTION_MIN_SAMPLES); research=ResearchEngine(store); learning=ClosedLoopLearning(store,evolution)
...
async def poll_once():
    started=time.perf_counter(); state["last_poll"]=time.time(); state["polls_total"]+=1; poll_no=state["polls_total"]; logger.info("POLL_START number=%s source=fusion interval_seconds=%s",poll_no,POLL_SECONDS)
    try:
        ...
        conflicts=data.get("verification_conflicts",[]) or []; conflict_ids={str(x.get("active_id")) for x in conflicts}
        for event in events:
        ...
        s24=data.get("verification",{}).get("scores24",{}) or {}; s24_added=store.add_external_observations("scores24",s24.get("observations",[]),s24.get("retrieved_at"))
        duration=round((time.perf_counter()-started)*1000,1); state.update({"items":len(events),"last_poll_items":stored,"last_success":time.time(),"last_error":None,"last_poll_duration_ms":duration,"polls_success":state["polls_success"]+1,"consecutive_failures":0}); logger.info("POLL_SUCCESS number=%s source=%s events=%s stored=%s scores24_observations=%s scores24_added=%s verification_conflicts=%s duration_ms=%s",poll_no,source,len(events),stored,len(s24.get("observations",[])),s24_added,len(conflicts),duration); return len(events)
...
async def polling_loop():
    ...
    while True:
        try: count=await poll_once(); backoff=POLL_SECONDS; state["next_poll_at"]=time.time()+backoff
...
async def lifespan(app):
    ...
    await asyncio.gather(*tasks,return_exceptions=True); state.update({"running":False,"evolution_running":False,"research_running":False,"learning_running":False}); await adapter.close()
...
async def record_umios_predictions(item:PredictionBatchIn):
    for p in item.predictions: store.add_prediction(prediction_id=p.prediction_id,fixture_id=p.fixture_id,market=p.market,predicted_probability=p.predicted_probability,selection=p.selection,odds=p.odds,model_version=p.model_version,features=p.features)
    logger.info("UMIOS_PREDICTIONS_INGESTED count=%s",len(item.predictions)); return {"ok":True,"source":"umios","recorded":len(item.predictions),"prediction_ids":[p.prediction_id for p in item.predictions]}
