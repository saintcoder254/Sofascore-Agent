...
class FeedFusionAdapter:
    """Multi-source football acquisition with ordered failover and independent challengers."""
    ...
    def __init__(self, sofascore_base, timeout=15):
        ...
        self.active_source = None; self.last_fallback_error = None
    ...
    async def _fallback_espn(self):
        ...
        try:
            response=await self.fallback_client.get(self.ESPN_BASE,params={"dates":day}); response.raise_for_status(); normalized=self._normalize_espn(response.json())
            if not normalized["events"]: raise RuntimeError("ESPN returned no match events")
            self.metrics["espn_success"]+=1; self.metrics["last_source"]="espn"; self.metrics["last_success_at"]=time.time(); self.metrics["last_error"]=None; self.active_source="espn"; logger.warning("FUSION_FALLBACK_SUCCESS source=espn events=%s",len(normalized["events"])); return normalized
    ...
    async def _fallback_fotmob(self):
        ...
        try:
            normalized=await self.fotmob.today_events(); self.metrics["fotmob_success"]+=1; self.metrics["last_source"]="fotmob"; self.metrics["last_success_at"]=time.time(); self.metrics["last_error"]=None; self.active_source="fotmob"; logger.warning("FUSION_FALLBACK_SUCCESS source=fotmob events=%s",len(normalized["events"])); return normalized
        except Exception as exc:
    ...
    async def verify_futbol24(self):
        ...
        try:
            payload=await self.futbol24.today_events(); events=payload.get("events",[]); self.metrics["futbol24_success"]+=1; self.metrics["futbol24_verification_events"]=len(events); self.metrics["last_futbol24_at"]=time.time(); logger.info("FUTBOL24_VERIFY_SUCCESS events=%s",len(events)); return payload
        except Exception as exc:
            self.metrics["futbol24_failures"]+=1; logger.warning("FUTBOL24_VERIFY_FAILED error=%r",exc); return {"source":"futbol24","source_priority":4,"retrieved_at":time.time(),"date":self._date(),"events":[],"error":repr(exc)}

    async def query_scores24(self):
        self.metrics["scores24_attempts"]+=1
        payload=await self.scores24.today_predictions(); observations=payload.get("observations",[]); self.metrics["scores24_observations"]=len(observations)
        if observations: self.metrics["scores24_success"]+=1; self.metrics["last_scores24_at"]=time.time()
    ...
    def reconcile_scores(self, active_payload, verification_payload):
        conflicts=[]; active_events=active_payload.get("events",[]) if active_payload else []; verify_events=verification_payload.get("events",[]) if verification_payload else []
        ...
        if conflicts: logger.warning("FUTBOL24_SCORE_CONFLICTS count=%s",len(conflicts))
        return conflicts

    async def today_events(self):
        self.metrics["primary_attempts"]+=1
        try:
            payload=await self.primary.today_events(); payload["source"]="sofascore"; payload["source_priority"]=1; payload["retrieved_at"]=time.time(); self.metrics["primary_success"]+=1; self.metrics["last_source"]="sofascore"; self.metrics["last_success_at"]=time.time(); self.metrics["last_error"]=None; self.active_source="sofascore"; logger.info("FUSION_PRIMARY_SUCCESS source=sofascore events=%s",len(payload.get("events",[])))
        except Exception as primary_exc:
            self.metrics["primary_failures"]+=1; self.metrics["last_error"]=repr(primary_exc); logger.warning("FUSION_PRIMARY_FAILED source=sofascore error=%r; trying espn",primary_exc)
            try: payload=await self._fallback_espn()
                ...
                except Exception as fotmob_exc: self.last_fallback_error=repr(fotmob_exc); logger.error("FUSION_ALL_SOURCES_FAILED sofascore=%r espn=%r fotmob=%r",primary_exc,espn_exc,fotmob_exc); raise RuntimeError("All football acquisition sources failed") from fotmob_exc
        verification=await self.verify_futbol24(); scores24=await self.query_scores24(); payload["verification"]={"futbol24":verification,"scores24":scores24}; payload["verification_conflicts"]=self.reconcile_scores(payload,verification)
        return payload

    def payload_hash(self,payload): return self.primary.payload_hash(payload)
    async def close(self):
        await self.primary.close(); await self.fotmob.close(); await self.futbol24.close(); await self.scores24.close(); await self.fallback_client.aclose()
