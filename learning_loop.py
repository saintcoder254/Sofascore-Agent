...
class ClosedLoopLearning:
    ...
    def _team_result(event):
        ...
        if state not in {"post", "final", "finished", "completed"}:
            return None
    ...
    def _market_outcome(cls, row, result):
        ...
        if market in {"over under", "o/u", "goals", "total goals", "over/under"}:
            ...
            if "over" in selection or selection.startswith("o"): return float(total > line)
            if "under" in selection or selection.startswith("u"): return float(total < line)
    ...
    def run(self, reason="scheduled"):
        ...
        logger.info("LEARNING_CYCLE reason=%s resolved=%s unresolved=%s samples=%s status=%s roi=%s", reason, resolution["resolved"], resolution["unresolved"], evolution["baseline"]["samples"], evolution["status"], performance.get("roi"))
